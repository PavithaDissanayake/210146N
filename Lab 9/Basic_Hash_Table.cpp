...
struct HashTable{
    ...
    void insert(string user_name,string user_password){
        ...
        if (empty) { // checks if the slot is empty
            password[hash] = user_password; // inserts if empty
            cout << "User Inserted\n";
        }
        else { // if the slot is already occupied
    ...
    void hash_lookup(string user_name){
        ...
        hash = hashfunc(user_name);
        empty = is_slot_empty(hash);
        if (empty) { // if the item looking for is not in the table
            cout << "No item found\n";
        }
    ...
    void delete_item(string user_name){
        ...
        if(empty){ // if the item looking for is not in the table
            ...
            password[hash].clear(); // deletes if available
            cout<<"User deleted\n";
        }
